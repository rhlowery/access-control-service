...
describe('RequestService', () => {
  ...
  it('throws on fetch error', async () => {
    ...
    await expect(RequestService.getRequests()).rejects.toThrow(/HTTP error!/);
  });
  ...
  it('throws on approve error', async () => {
    ...
    await expect(RequestService.approveRequest('1')).rejects.toThrow(/HTTP error!/);
  });

  it('throws on reject error', async () => {
    mockFetch.mockResolvedValueOnce(new Response(null, { status: 400 }));
    await expect(RequestService.rejectRequest('1', 'reason')).rejects.toThrow(/HTTP error!/);
  });

  it('throws on verify error', async () => {
    mockFetch.mockResolvedValueOnce(new Response(null, { status: 400 }));
    await expect(RequestService.verifyRequest('1')).rejects.toThrow(/HTTP error!/);
  });
